...
from .data_utils import generate_partial_point_slice, sample_points
import cv2
...
class Shapes(torch.utils.data.Dataset):
    ...
    def modify_commandline_options(parser):
        ...
        parser.add_argument("--min_partial_ratio", type=float, default=0.25)
        parser.add_argument("--max_partial_ratio", type=float, default=0.9)
        parser.add_argument("--test_partial_ratio", type=float, default=0.5)
    ...
    def get_item_by_name(self, name, sample_mesh=False, load_mesh=False, load_partial=False):   
        ...
        if sample_mesh or generate_point_file:
            if load_partial:
                ...
                partial_num_point = int(keep_ratio * num_point) + 1 if self.opt.split == 'train' else self.opt.num_point
